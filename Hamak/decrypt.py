...
def decrypt_text_hamak(encrypted_text, private_key):
    """
    Deszyfruje tekst za pomocą prostego przesunięcia bajtowego.
    """
    decrypted_text = bytearray()
    for char in encrypted_text:
        decrypted_char = (char - private_key) % 256
        decrypted_text.append(decrypted_char)
    return decrypted_text.decode('utf-8')  # Dekodowanie bajtów do tekstu

def decrypt_file_hamak(input_path, output_path, private_key):
    """
    ...
    try:
        with open(temp_file, "rb") as infile, open(output_path, "wb") as outfile:
            while chunk := infile.read(1024):
                decrypted_chunk = bytes((byte - private_key) % 256 for byte in chunk)
